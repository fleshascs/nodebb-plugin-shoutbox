...
Shouts.addShout = function(fromuid, content, callback) {
	NodeBB.db.incrObjectField('global', 'nextSid', function(err, sid) {
		...
		async.parallel([
			async.apply(NodeBB.db.setObject, 'shout2:' + sid, shout),
			async.apply(NodeBB.db.listAppend, 'shouts2', sid)
		], function(err) {
			if (err) return callback(err);
...
Shouts.getShouts = function(start, end, callback) {
	NodeBB.db.getListRange('shouts2', start, end, function(err, sids) {
		if (err) return callback(err);
...
Shouts.removeShout = function(sid, uid, callback) {
	async.parallel({
		...
		fromUid: async.apply(NodeBB.db.getObjectField, 'shout2:' + sid, 'fromuid')
	}, function(err, result) {
		if (err) return callback(err);

		if (result.isAdmin || result.isMod || parseInt(result.fromUid, 10) === parseInt(uid, 10)) {
			NodeBB.db.setObjectField('shout2:' + sid, 'deleted', '1', function (err, result) {
				if (err) return callback(err);
...
Shouts.editShout = function(sid, msg, uid, callback) {
	async.parallel({
		...
		if (result.isAdmin || result.isMod || parseInt(result.fromUid, 10) === parseInt(uid, 10)) {
			NodeBB.db.setObjectField('shout2:' + sid, 'content', msg, function(err, result) {
				if (err) return callback(err);
...
Shouts.pruneDeleted = function(uid, callback) {
	NodeBB.User.isAdministrator(uid, function(err, isAdmin) {
		...
		NodeBB.db.getListRange('shouts2', 0, -1, function(err, sids) {
			...
			NodeBB.db.getObjectsFields(keys, ['sid', 'deleted'], function(err, items) {
				async.map(items, function(item, next) {
					...
					NodeBB.db.listRemoveAll('shouts2', item.sid, function(err, result) {
						next(null, item.sid);
					});
				}, function(err, sids) {
					var keys = sids.map(function(sid) {
	...
	NodeBB.User.isAdministrator(uid, function(err, isAdmin) {
		...
		NodeBB.db.getListRange('shouts2', 0, -1, function (err, sids) {
			...
			async.parallel([
				...
				async.apply(NodeBB.db.delete, 'shouts2'),
